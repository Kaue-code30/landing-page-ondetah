        ...
        <div className="relative w-full">
            ...
            <AnimatePresence>
                <motion.header
                    ...
                    animate={{
                        background: onScroll
                            ? "rgba(40, 38, 91, 1) 0%"  // Cor sólida
                            : `rgba(40, 38, 91, ${closeIcon ? "1" : "0"})`  // Transição suave para transparente
                    }}
                ...
                >
                    <div className="w-[90%] h-full justify-between items-center flex">
                        ...
                        <ul className={` lg:flex items-center justify-center hidden  gap-5`}>
                            ...
                            <li onClick={() => { setLocalizarPedido(!localizarPedido) }} className="border cursor-pointer hover:bg-textSecondColor transition hover:text-textPrimaryColor hover:scale-95 text-white rounded-lg px-4 p-2">
                                Rastrear pedido
                            </li>
                        </ul>
                        <motion.div
                            initial={{ opacity: 0, translateY: -100 }}
                            animate={{ opacity: closeIcon ? 1 : 0, translateY: closeIcon ? 0 : -20 }}
                            transition={{ duration: 0.7, ease: "backInOut" }}
                            className={`absolute top-[79px] left-0 w-full bg-primaryColor lg:hidden flex flex-col gap-8  pb-10 py-5 ${closeIcon ? "flex" : "hidden"}`}
                        >
                        ...
                        <div onClick={() => handleOpenMenu()} className="w-8  lg:hidden visible h-8 ">
