    ...
    return (
        <section id="planos" className={` ${openModal ? "bg-primaryColor lg:mt-10" : "bg-white"} flex items-center pb-20 justify-center flex-col lg:h-auto max-w-[1996px] lg:pt-[20px] lg:pb-[100px]  w-full  `}>
            <motion.div initial={{ opacity: 0, y: 200 }}
                ...
                <button onClick={() => { handleCloseModal() }} className={`lg:w-[130px] hover:scale-95 transition duration-300 w-[30%] ${openModal === false ? "hidden" : "flex"} h-10 flex items-center mb-5 bg-white justify-center rounded-lg`}>
                    <IoIosArrowRoundBack fontSize={35} /> voltar
                </button>

                <h1 className={`text-3xl lg:text-4xl text-left lg:text-center  leading-tight  ${openModal ? "text-white" : "text-textPrimaryColor"}  font-bold`}>
                    Escolha o melhor plano para você:
                </h1>
                <h2 className={` ${openModal ? "text-white" : "text-textPrimaryColor"} pt-3 leading-tight lg:text-center text-left  w-full font-normal text-base`}>
                    Nossos planos são do tamanho das necessidades do seu negócio.
                </h2>
                <h2 className={` pt-3 lg:hidden leading-tight lg:text-center text-left  w-full font-normal text-sm ${openModal ? "text-white hidden" : "text-textPrimaryColor"} `}>
